...
-------------------------------------------------------------------*/
  ...
  function mapInit() {
    ...
    const map = L.map(target).setView([14.7389, -17.5127], 15);
    ...
    L.marker([14.73825, -17.512384]).addTo(map).openPopup();
  }
...
const roleTiltle = document.querySelector(".role_title");
...
sideBarItem.forEach((item) => {
  if (roleTiltle.innerText === item.innerText) {
...
for (let i = 0; i < roleForm.length; i++) {
  const input = roleForm[i];
  input.addEventListener("keyup", (e) => {
    replace.forEach((span) => {
      if (span.id === input.getAttribute("name")) {
...
let button = document.getElementById('button');let civilite = document.querySelector('[name="civilite"]');let civilite_validator = document.querySelector('.civilite_validator');let prenom = document.querySelector('[name="prenom"]');let prenom_validator = document.querySelector('.prenom_validator');let nom = document.querySelector('[name="nom"]');let nom_validator = document.querySelector('.nom_validator');let date_naiss = document.querySelector('[name="date_naiss"]');let date_naiss_validator = document.querySelector('.date_naiss_validator');let nationalite = document.querySelector('[name="nationalite"]');let nationalite_validator = document.querySelector('.nationalite_validator');let adress = document.querySelector('[name="adress"]');let adress_validator = document.querySelector('.adress_validator');let ville = document.querySelector('[name="ville"]');let ville_validator = document.querySelector('.ville_validator');let mail = document.querySelector('[name="mail"]');let mail_validator = document.querySelector('.mail_validator');let tel = document.querySelector('[name="tel"]');let tel_validator = document.querySelector('.tel_validator');let diplome = document.querySelector('[name="diplome"]');let diplome_validator = document.querySelector('.diplome_validator');let date_dd = document.querySelector('[name="date_dd"]');let date_dd_validator = document.querySelector('.date_dd_validator');let matiere = document.querySelector('[name="matiere"]');let matiere_validator = document.querySelector('.matiere_validator');let niveau = document.querySelector('[name="niveau"]');let niveau_validator = document.querySelector('.niveau_validator');let cv = document.querySelector('[name="cv"]');let cv_validator = document.querySelector('.cv_validator');let civiliteList = [civilite, civilite_validator,];let prenomList = [prenom, prenom_validator,];let nomList = [nom, nom_validator,];let date_naissList = [date_naiss, date_naiss_validator,];let nationaliteList = [nationalite, nationalite_validator,];let adressList = [adress, adress_validator,];let villeList = [ville, ville_validator,];let mailList = [mail, mail_validator,];let telList = [tel, tel_validator,];let diplomeList = [diplome, diplome_validator,];let date_ddList = [date_dd, date_dd_validator,];let matiereList = [matiere, matiere_validator,];let niveauList = [niveau, niveau_validator,];let cvList = [cv, cv_validator,];
let inputNaNList = [civiliteList, prenomList, nomList, nationaliteList, villeList, diplomeList, matiereList, ];let inputList = [date_naissList, adressList, mailList, telList, date_ddList, niveauList];

cv.addEventListener('focus', () => {
  cv_validator.textContent = "";
});

cv.addEventListener('keyup', () => {
  if (cv.value == "") {
    cv_validator.textContent = "Veuillez importer votre CV";
...
inputNaNList.forEach(input => {
  let name = input[0];
  let validator = input[1];
  notEmptyNaN(name, validator)
});

inputList.forEach(input => {
  let name = input[0];
  let validator = input[1];
  notEmpty(name, validator)
});

function notEmpty(input, input_validator) {
  
  input.addEventListener('focus', () => {
    input_validator.textContent = "";
  });

  input.addEventListener('keyup', () => {
    if (input.value == "") {
      input_validator.textContent = "Veuillez remplir ce champ";
      button.disabled = true;
    } else {
      input_validator.textContent = "";
      button.disabled = false;
...
function notEmptyNaN(input, input_validator) {
  input.addEventListener('focus', () => {
    input_validator.textContent = "";
  });
  ...
  input.addEventListener('keyup', () => {
    if (input.value == "") {
      ...
      input_validator.textContent = "Veuillez ne pas saisir de chiffre";
    } else {
      input_validator.textContent = "";
      button.disabled = false;
    }
...
// un se est un programme qui permet a l'utilisateur d'exploiter les ressources materielles d'un ordinateur
